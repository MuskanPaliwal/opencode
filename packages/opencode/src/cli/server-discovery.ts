...
export interface Interface {
  readonly write: (url: URL) => Effect.Effect<void, unknown>
  readonly remove: () => Effect.Effect<void, unknown>
  readonly find: () => Effect.Effect<string | undefined, unknown>
}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const read = () =>
      fs
        .readJson(file)
        .pipe(Effect.catch(() => Effect.succeed(undefined)), Effect.map((entry) => Option.getOrUndefined(decodeEntry(entry))))
    ...
    return Service.of({
      write: (url) => fs.writeJson(file, { url: localURL(url).toString(), pid: process.pid }, 0o600).pipe(Effect.orDie),
      remove,
      find: () =>
        Effect.gen(function* () {
          const entry = yield* read()
